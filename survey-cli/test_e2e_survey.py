...
def step1_start_chrome_and_inject():
        ...
        for pid_line in result.stdout.strip().split("\n"):
            ...
            if pid_str.isdigit():
                try:
                    ...
                    if "/tmp/heypiggy" in cmdline or f"remote-debugging-port={CHROME_PORT}" in cmdline:
    ...
    for i in range(15):
        try:
            v = json.loads(urllib.request.urlopen(f"http://127.0.0.1:{CHROME_PORT}/json/version", timeout=2).read())
...
def inject_cookies_into_ws(ws_url: str) -> bool:
    ...
    cookie_names = {"PHPSESSID", "user_session", "user_id", "user_a_b_group", "lang_pig", "g_state", "referer"}
    heypiggy = [
        {"name": c["name"], "value": c["value"], "domain": c.get("domain", "www.heypiggy.com"),
...
def step2_verify_dashboard():
    ...
    pages = json.loads(urllib.request.urlopen(f"http://127.0.0.1:{CHROME_PORT}/json", timeout=5).read())
    dash = next((p for p in pages if "dashboard" in p.get("url", "").lower() and p.get("type") == "page"), None)
    if not dash:
...
def step3_inject_cookies(ws_url: str):
    ...
    try:
        ...
        ws.send(json.dumps({"id": 1, "method": "Network.enable"}))
        json.loads(ws.recv())
        ws.send(json.dumps({"id": 2, "method": "Network.setCookies", "params": {"cookies": heypiggy}}))
        r = json.loads(ws.recv())
        ws.close()
        success = r.get("result", {}).get("success") is True
...
"""}
        ...
        print(f"  Modals: {data.get('modals', 0)}, Start buttons: {len(data.get('startButtons', []))}")
        
        for btn in data.get("startButtons", []):
            print(f"    - {btn.get('text')} | onclick={btn.get('onclick','none')[:80]} | href={btn.get('href','')[:80]}")
        
        # Check if survey tab opened
        pages = json.loads(urllib.request.urlopen(f"http://127.0.0.1:{CHROME_PORT}/json", timeout=5).read())
        non_dash = [p for p in pages if "dashboard" not in p.get("url", "").lower() and "about:blank" not in p.get("url", "")]
...
def step7_run_survey_loop(survey_ws: str, survey_id: str):
    ...
    try:
        ...
        print(f"  Profile: {profile.get('age')}yo {profile.get('gender')} from {profile.get('city')}")
        ...
        try:
            ...
            print(f"  Balance before/after: €{result.balance_before:.2f} → €{result.balance_after:.2f}")
