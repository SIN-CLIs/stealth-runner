...
def _solve_text(ws_url: str) -> dict:
    ...
    if not nvidia_key:
        return {"status": "error", "reason": "no_nvidia_key"}
    ...
    try:
            ...
            data=json.dumps(
                {
                    ...
                    "messages": [
                        {
                            "role": "user",
                            "content": f"What text is shown? Reply ONLY with the text.\n![img](data:image/png;base64,{b64})",
                        }
                    ],
                    "max_tokens": 20,
                    "temperature": 0.1,
        ...
        if not text:
            return {"status": "error", "reason": "empty_ocr"}
    except Exception as e:
    ...
    try:
        ws = websocket.create_connection(ws_url, timeout=10)
        ws.send(
            json.dumps({"id": 0, "method": "Runtime.evaluate", "params": {"expression": type_js}})
        )
...
def _solve_slide(ws_url: str) -> dict:
    ...
    try:
        ...
        if not geo:
            return {"status": "error", "reason": "no_slider"}
    except Exception as e:
        return {"status": "error", "reason": str(e)[:100]}
    ...
    for i in range(41):
        ...
        px = (
            sx
            + (ex - sx) * t
            - (20 * (1 - abs(2 * t - 1))) * (sy > ey and 1 or -1 if ex != sx else 0)
        )
        py = sy + (ey - sy) * t
...
def _registry(cmd: str, ok: bool, details: dict):
    try:
        ...
        reg = CommandRegistry()
        reg.record_command(cmd, ok, details)
    except Exception:
        pass
...
def _preflight(cmd: str) -> bool:
    try:
        ...
        CommandRegistry().validate_command(cmd)
        return True
    except Exception:
        return True
...
def solve(ws_url: str, captcha_type: str = "auto") -> dict:
    ...
    """
    if not _preflight("solve_captcha"):
        return {"status": "error", "reason": "preflight_failed"}
    if captcha_type == "auto":
        captcha_type = _detect_type(ws_url)
    ...
    if captcha_type in ("text", "visual"):
        result = _solve_text(ws_url)
    elif captcha_type == "slide":
        result = _solve_slide(ws_url)
    else:
        result = {"status": "error", "reason": f"unknown_type: {captcha_type}"}
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: tool_solve_captcha.py <ws_url> [type]")
    else:
        print(
            json.dumps(solve(sys.argv[1], sys.argv[2] if len(sys.argv) > 2 else "auto"), indent=2)
        )
