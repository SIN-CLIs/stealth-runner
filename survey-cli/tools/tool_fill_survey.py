...
def load_profile(name: str = "jeremy_schulze") -> Dict:
    ...
    profile["_norm"] = {k: _normalize(v) for k, v in profile.items() if isinstance(v, str)}
    ...
    def decide_actions(self, snapshot: Dict) -> List[Dict]:
        ...
        for q_idx, question in enumerate(questions):
            ...
            if key == "age":
                ...
                income = self.profile.get("household_income") or self.profile.get(
                    "personal_income", ""
                )
                if income:
