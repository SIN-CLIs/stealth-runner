...
pytestmark = pytest.mark.skip(
    reason="SR-63 #62: E2E test requires real browser + live CDP; not suitable for CI"
)
# === END SR-63 skip ===
...
async def test_open_survey():
    ...
    try:
        version = json.loads(
            subprocess.run(
                ["curl", "-s", "http://127.0.0.1:9999/json/version"], capture_output=True, text=True
            ).stdout
        )
    ...
    pages = json.loads(
        subprocess.run(
            ["curl", "-s", "http://127.0.0.1:9999/json/list"], capture_output=True, text=True
        ).stdout
    )
    dashboard = next(
        (p for p in pages if "dashboard" in p.get("url", "") and p.get("type") == "page"), None
    )
    if not dashboard:
    ...
    print(
        f"[3] Cookies to inject: {len(heypiggy_cookies)} (PHPSESSID, user_session, user_id, etc.)"
    )
...
""",
        ...
        target_resp = subprocess.run(
            ["curl", "-s", "-X", "PUT", f"http://127.0.0.1:9999/json/new?{survey_url}"],
            capture_output=True,
            text=True,
        )
