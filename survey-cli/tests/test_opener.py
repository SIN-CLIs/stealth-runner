...
pytestmark = pytest.mark.skip(
    reason="SR-63 #62: mock drift — SurveyOpener mocks need update for current chrome helpers"
)
# === END SR-63 skip ===
...
class TestSurveyOpenerOpenNewTab(unittest.TestCase):
    ...
    @patch(
        "survey.opener.chrome.create_blank_tab",
    ...
    @patch(
        "survey.opener.SurveyOpener._find_survey_tab_ws",
        return_value=("ws://survey", "https://survey.example.com"),
    )
    def test_open_new_tab_returns_target(
        self, mock_find, mock_activate, mock_nav, mock_inj, mock_create
    ):
    ...
    @patch("survey.opener.chrome.activate_tab")
    def test_create_blank_tab_fallback(self, mock_activate, mock_create_tab, mock_blank):
        """Fallback to chrome.create_tab when blank tab fails."""
        opener = SurveyOpener(cdp_port=9999, debug=False)
        with patch.object(opener, "_find_survey_tab_ws", return_value=("ws://s", "https://s.com")):
...
class TestSurveyOpenerOpenInPageModal(unittest.TestCase):
    ...
    @patch("survey.opener.SurveyOpener._click_survey_card", return_value="ws://dashboard")
    @patch("survey.opener.SurveyOpener._pre_survey_cleanup", return_value=0)
    def test_open_in_page_returns_dashboard_ws(self, mock_cleanup, mock_click, mock_tabs):
        """In-page modal returns dashboard WS."""
        opener = SurveyOpener(cdp_port=9999, debug=False)
        result = opener.open(
            "s1", "in_page_modal", "in-page://modal", dashboard_ws="ws://dashboard"
        )
        self.assertIsNotNone(result.target)
    ...
    @patch(
        "survey.opener.chrome.find_bot_tabs",
        return_value=[{"id": "t1", "webSocketDebuggerUrl": "ws://dash"}],
    )
    @patch("survey.opener.SurveyOpener._click_survey_card", return_value="ws://dash")
    @patch("survey.opener.SurveyOpener._find_new_tab_after_click", return_value="ws://new_tab")
    @patch("survey.opener.SurveyOpener._pre_survey_cleanup", return_value=0)
    def test_open_in_page_detects_new_tab(self, mock_cleanup, mock_new_tab, mock_click, mock_tabs):
        """If clickSurvey opens a new tab, mode becomes 'redirect'."""
        opener = SurveyOpener(cdp_port=9999, debug=False)
        result = opener.open("s1", "in_page_modal", "in-page://modal", dashboard_ws="ws://dash")
...
class TestSurveyOpenerErrorHandling(unittest.TestCase):
    ...
    @patch(
        "survey.opener.chrome.create_blank_tab", return_value={"id": "tab-x", "ws_url": "ws://t"}
    )
    @patch("survey.opener.chrome.inject_stealth_to_tab", return_value=True)
    @patch("survey.opener.chrome.navigate_tab", return_value=True)
    @patch("survey.opener.chrome.activate_tab")
    @patch(
        "survey.opener.SurveyOpener._find_survey_tab_ws", return_value=("ws://t", "https://s.com")
    )
    @patch(
        "survey.opener.SurveyOpener._read_page_text",
        return_value="still loading just getting things ready please wait",
    )
    @patch("survey.opener.SurveyOpener._close_tab")
    def test_stuck_loading_returns_screen_out(
        self, mock_close, mock_text, mock_find, mock_activate, mock_nav, mock_inj, mock_create
    ):
    ...
    @patch("survey.opener.SurveyOpener._close_tab")
    def test_expired_survey_returns_screen_out(
        self, mock_close, mock_text, mock_find, mock_activate, mock_nav, mock_inj, mock_create
    ):
...
class TestSurveyOpenerCloseAndRefresh(unittest.TestCase):
    ...
    def test_close_noop_for_in_page(self, mock_tabs):
        ...
        opener = SurveyOpener(cdp_port=9999, debug=False)
        target = SurveyTarget(
            survey_id="s1", provider="in_page_modal", ws_url="ws://dash", mode="in_page"
        )
        opener.close(target)  # should not raise

    @patch("survey.opener.chrome.activate_tab")
    @patch("survey.opener.chrome.get_ws_for_tab", return_value="ws://fresh")
    @patch("survey.opener.SurveyOpener._refresh_tab_ws", return_value="ws://fresh")
    def test_refresh_ws_returns_fresh_url(self, mock_refresh, mock_get, mock_activate):
        """refresh_ws returns new WS URL when tab navigated."""
        opener = SurveyOpener(cdp_port=9999, debug=False)
        target = SurveyTarget(
            survey_id="s1", provider="qualtrics", ws_url="ws://old", tab_id="tab-x"
        )
