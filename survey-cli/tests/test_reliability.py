...
class TestRetryPolicy(unittest.TestCase):
    ...
    def test_success_no_retry(self):
        ...
        result = asyncio.get_event_loop().run_until_complete(policy.run(succeeds))
    ...
    def test_transient_retry_then_success(self):
        ...
        result = asyncio.get_event_loop().run_until_complete(policy.run(fails_twice_then_succeeds))
    ...
    def test_permanent_no_retry(self):
        ...
        with self.assertRaises(PermanentError):
            asyncio.get_event_loop().run_until_complete(policy.run(fails_permanent))
    ...
    def test_max_attempts_exceeded(self):
        ...
        with self.assertRaises(TransientError):
            asyncio.get_event_loop().run_until_complete(policy.run(always_transient))
...
class TestDefaultClassify(unittest.TestCase):
    ...
    def test_account_banned_is_permanent(self):
        self.assertEqual(default_classify(Exception("account banned")), Retryability.PERMANENT)

    def test_503_is_transient(self):
        self.assertEqual(
            default_classify(Exception("HTTP 503 Service Unavailable")), Retryability.TRANSIENT
        )
...
class TestContradictionDetector(unittest.TestCase):
    ...
    def test_categorize_german(self):
        ...
        self.assertEqual(self.detector.categorize("Wie alt sind Sie?"), IdentityCategory.AGE)
        self.assertEqual(
            self.detector.categorize("Was ist Ihr Geschlecht?"), IdentityCategory.GENDER
        )
        self.assertEqual(
            self.detector.categorize("Ihr monatliches Einkommen?"), IdentityCategory.INCOME
        )

    def test_categorize_english(self):
        """Categorize detects English keywords."""
        self.assertEqual(self.detector.categorize("What is your age?"), IdentityCategory.AGE)
        self.assertEqual(self.detector.categorize("Select your gender"), IdentityCategory.GENDER)
        self.assertEqual(
...
class TestWebhook(unittest.TestCase):
    ...
    def test_webhook_env_set_fires(self):
        ...
        try:
            ...
            dlq = DLQ(dlq_path=temp_dir, webhook_url=f"http://127.0.0.1:{port}/webhook")
