            ...
            json.dumps(
                {
                    ...
                    "params": {
                        "expression": "var b=document.querySelector('button');if(b){var r=b.getBoundingClientRect();return JSON.stringify({x:r.x+r.width/2,y:r.y+r.height/2});}return'{}';"
                    },
                }
            )
        )  # noqa: E501
            ...
            json.dumps(
                {
                    ...
                    "method": "Runtime.evaluate",
                    "params": {
                        "expression": "var b=document.querySelector('button');if(b){b.click();return'clicked';}return'none';"
                    },
                }
            )
        )  # noqa: E501
            ...
            json.dumps(
                {
                    ...
                    "method": "Runtime.evaluate",
                    "params": {
                        "expression": "var t=document.querySelector('textarea');if(t){t.value='ROBOT I enjoy sharing my opinion about this topic.';t.dispatchEvent(new Event('input',{bubbles:true}));t.dispatchEvent(new Event('change',{bubbles:true}));return'filled';}return'none';"
                    },
                }
            )
        )  # noqa: E501
...
"""
            ...
            messages=[
                {
                    ...
                    "content": [
                        {
                            "type": "text",
                            "text": "This is a captcha image. Read ONLY the character sequence shown in the image. "  # noqa: E501
        ...
        if len(captcha) < 3:
            return {
                "success": False,
                "error": f"OCR too short ({len(captcha)} chars): '{captcha}'",
                "raw": raw,
            }  # noqa: E501
        if len(captcha) > 10:
            captcha = captcha[:10]
...
def shadow_dom_click(ws_url, selector, tag_hint="", debug=False):
    ...
    if debug:
        print(
            f"  [SHADOW] Clicking {element.get('tag', '?')}>{element.get('targetTag', '?')} "
...
def shadow_dom_fill(ws_url, selector, value, tag_hint="", debug=False):
    ...
    if debug:
        print(
            f"  [SHADOW] Filling {element.get('tag', '?')}>{element.get('targetTag', '?')} "
            f"with '{value[:20]}...'"
        )
...
def navigate_purespectrum_shadow_dom(ws_url, max_steps=15, debug=False):
    ...
    for step in range(max_steps):
        ...
        if any(
            x in text
            for x in ["leider", "nicht geeignet", "disqualif", "screenout", "nicht qualifiziert"]
        ):  # noqa: E501
            if debug:
        ...
        for selector, tag in button_selectors:
            ...
            if btn and not btn.get("disabled"):
                ...
                if any(
                    x in btn_text
                    for x in ["nächste", "weiter", "next", "submit", "abschicken", "fertig"]
                ):  # noqa: E501
                    if debug:
        ...
        if not clicked:
            if debug:
                print(
                    f"  [SHADOW] Page {step}: No clickable elements found — page might be complete"
                )  # noqa: E501
            ...
            if not any(x in text for x in ["bitte legen", "zahl", "puzzle"]):
                return {
                    "status": "unknown",
                    "pages": step,
                    "error": "No Shadow DOM elements found and no completion markers",
                }  # noqa: E501
