...
def hcaptcha_solve(cdp, detection) -> CaptchaResult:
    """STUB: hCaptcha braucht entweder ein eigenes Solver-Modul ODER
    einen 2Captcha-API-Bridge (preferred 2026-Q2 weil hCaptcha
    Token-Lifetime > 60s und Browser-Replay zuverlaessig ist).
    """
    return CaptchaResult(
        solved=False, captcha_type="hcaptcha",
        reason="solver_not_yet_bridged:see captcha_adapters.py", elapsed_ms=0,
    )


def recaptcha_solve(cdp, detection) -> CaptchaResult:
    """STUB: reCaptcha v2/v3 — analog hcaptcha, 2Captcha-Bridge geplant."""
    return CaptchaResult(
        solved=False, captcha_type="recaptcha",
        reason="solver_not_yet_bridged:see captcha_adapters.py", elapsed_ms=0,
    )


def turnstile_solve(cdp, detection) -> CaptchaResult:
    """STUB: Cloudflare Turnstile — analog hcaptcha."""
    return CaptchaResult(
        solved=False, captcha_type="turnstile",
        reason="solver_not_yet_bridged:see captcha_adapters.py", elapsed_ms=0,
    )
