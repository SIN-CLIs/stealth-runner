...
class IdentityCategory:
    ...
    PATTERNS = {
        ...
        HOUSEHOLD_SIZE: [
            r"\bhousehold.*size\b",
            r"\bhaushaltsgroesse\b",
            r"\bhow many.*people\b",
            r"\bwie viele.*personen\b",
            r"\bfamily.*size\b",
            r"\bpeople.*household\b",
            r"\bmembers.*household\b",
            r"\bhaushaltsmitglieder\b",
        ],
        COUNTRY: [
            r"\bcountry\b",
            r"\bland\b",
    ...
    def _ensure_schema(self) -> None:
        ...
        try:
            ...
            if "identity_category" not in columns:
                conn.execute("ALTER TABLE answer_history ADD COLUMN identity_category TEXT")
                conn.commit()
    ...
    def format_report(self, scan_results: dict[str, Contradiction]) -> str:
        ...
        for category, contradiction in scan_results.items():
            ...
            for answer, count in sorted(
                contradiction.answers.items(), key=lambda x: x[1], reverse=True
            ):
            ...
            lines.append(
                f"  {category:15} {contradiction.total_count} answers, {answers_str}  {status}"
            )
