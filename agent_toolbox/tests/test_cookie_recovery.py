...
async def test_full_recovery_flow():
    ...
    backup_result = CookieManager.create_backup(
        working_filename="test-cookies.json", working_dir="data"
    )
    ...
    if backup_count != restored_count:
        print("  FEHLER: Anzahl stimmt nicht überein!")
        await bm.close()
        return False
    ...
    # Simuliere: Wir erstellen ein totes Cookie-File (leere Cookies)

    # Wir können verify_session nicht einfach täuschen, aber wir können testen,
    # dass safe_save_cookies verify_session aufruft.
    # Stattdessen testen wir: Wenn verify_session False zurückgibt,
    # dann wird safe_save_cookies NICHT speichern.

    # Wir simulieren einen abgelaufenen Zustand durch ein leeres Cookie-File
    # und prüfen, dass verify_session das erkennen würde.
    ...
    # Wir können verify_session mit leeren Cookies nicht direkt testen,
    # da es auf dem Page-State basiert (nicht auf Cookie-Anzahl).
    # Aber wir können prüfen, dass die Logik korrekt ist:
    # - safe_save_cookies ruft verify_session auf
    # - Wenn False -> gibt error-Dict zurück (nicht speichert)
