...
from src.stealth_sync.semantic_engine import CATEGORIES, SemanticAnalyzer
...
class TestSemanticAnalyzer:
    ...
    def test_init_custom_parameters(self):
        ...
        analyzer = SemanticAnalyzer(
            api_key="test_key", base_url="https://custom.url/v1", model="custom-model"
        )
    ...
    def test_generate_doc_unit(self):
        ...
        with patch("src.stealth_sync.semantic_engine.OpenAI"):
            ...
            result = analyzer.generate_doc_unit("ses_test123", messages, classification)
    ...
    def test_generate_doc_unit_empty_messages(self):
        ...
        with patch("src.stealth_sync.semantic_engine.OpenAI"):
            ...
            result = analyzer.generate_doc_unit("ses_empty", [], classification)
...
class TestSemanticAnalyzerCategories:
    ...
    def test_classify_session_with_special_characters(self):
        ...
        with patch("src.stealth_sync.semantic_engine.OpenAI"):
            ...
            messages = [
                {
                    "role": "user",
                    "content": "Fix the bug in code.py line 42: def buggy_function():",
                },
                {
                    "role": "assistant",
                    "content": "Fixed with try-except block and proper error handling",
                },
                {
                    "role": "user",
                    "content": "Add new feature: async def fetch_data() -> List[Dict]:",
                },
        ...
        with patch("src.stealth_sync.semantic_engine.OpenAI"):
            ...
            for input_text, expected_category in test_cases:
                ...
                assert result["category"] == expected_category, (
                    f"Failed to parse '{input_text}' as {expected_category}"
                )
                assert result["confidence"] == 0.9
    ...
    def test_parse_classification_partial_matches(self):
        ...
        with patch("src.stealth_sync.semantic_engine.OpenAI"):
            ...
            result = analyzer._parse_classification(
                "valid text"
            )  # Can't pass None due to .lower() call
            assert result["category"] in CATEGORIES or result["category"] == "unknown"
    ...
    def test_build_classification_prompt_variations(self):
        ...
        with patch("src.stealth_sync.semantic_engine.OpenAI"):
            ...
            messages = [{"role": "user"}, {"role": "assistant", "content": "Valid message"}]
            prompt = analyzer._build_classification_prompt(messages)
    ...
    def test_generate_doc_unit_with_all_categories(self):
        ...
        with patch("src.stealth_sync.semantic_engine.OpenAI"):
            ...
            for category in CATEGORIES:
                ...
                doc_unit = analyzer.generate_doc_unit(f"ses_{category}", messages, classification)
...
async def test_async_compatibility():
    ...
    with patch("src.stealth_sync.semantic_engine.OpenAI"):
        ...
        doc_unit = analyzer.generate_doc_unit(
            "ses_test", messages, {"category": "test", "confidence": 0.5}
        )
        assert isinstance(doc_unit, dict)
