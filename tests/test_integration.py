...
class TestIntegrationPipeline:
    ...
    def test_full_pipeline_with_mocked_analysis(self, tmp_path):
        ...
        conn.execute(
            ...
            (
                ...
                int(time.time()),
                "user",
                "Fix the critical bug in the authentication module",
            ),
        )
        conn.execute(
            "INSERT INTO messages (session_id, timestamp, role, content) VALUES (?, ?, ?, ?)",
            (
                ...
                int(time.time()) + 1,
                "assistant",
                "Fixed the authentication issue with proper error handling",
            ),
        )
        conn.commit()
        conn.close()
    ...
    def test_pipeline_with_different_categories(self, tmp_path):
        ...
        conn = sqlite3.connect(str(db_path))
        conn.execute(
            "CREATE TABLE IF NOT EXISTS sessions (id TEXT PRIMARY KEY, created_at INTEGER, status TEXT, metadata TEXT)"
        )
        conn.execute(
            "CREATE TABLE IF NOT EXISTS messages (id INTEGER PRIMARY KEY AUTOINCREMENT, session_id TEXT, timestamp INTEGER, role TEXT, content TEXT, FOREIGN KEY (session_id) REFERENCES sessions(id))"
        )
...
async def test_async_pipeline_compatibility():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        OpenCodeDBPoller(db_path=f"{tmp_dir}/test.db")
        analyzer = SemanticAnalyzer()
