...
## 2026-05-02: Doctor Scan — 86 Fixes

- `fix.md`: `nvidia/nvidia/nemotron` → `nvidia/nemotron`
- `fix.md`: `cua-driver` → `skylight-cli`
- `fix.md`: `cua-driver` → `skylight-cli`
- `fix.md`: `cua-driver` → `skylight-cli`
- `fix.md`: `cua-driver` → `skylight-cli`
- `fix.md`: `cua-driver` → `skylight-cli`
- `fix.md`: `cua-driver` → `skylight-cli`
- `fix.md`: `cua-driver` → `skylight-cli`
- `banned.md`: `pgrep Chrome` | Greift Nutzer-Chrome statt isolier`→`playstealth launch (isolierte PID)`
- `banned.md`: `pkill Chrome` | Killt Nutzer-Prozesse, Datenverlus`→`NIEMALS – BANNED`
- … und 76 weitere
...
- **Fix**: skylight-cli nutzen (hat `--window-id` + `--element-index` Support)
  ...
  skylight-cli click --pid 42296 --window-id 30380 --element-index 35  # → Weiter im Popup!
  ```

- **Dateien**: `runner/trio_live.py`, `banned.md`, `brain.md`
- **Status**: ✅ skylight-cli mit window-id fix
...
- **Bug**: `nvidia/nemotron-3-nano-omni-30b-a3b-reasoning`
...
## skylight-cli: Wann erlaubt, wann nicht

- ✅ **ERLAUBT**: `skylight-cli get_window_state --pid --window-id`
- ✅ **ERLAUBT**: `skylight-cli click --pid --window-id --element-index`
- ❌ **BANNED**: `skylight-cli click --x --y` (Koordinatenraten)
