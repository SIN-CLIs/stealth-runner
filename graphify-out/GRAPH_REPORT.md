...
## Corpus Check
- 170 files · ~2,728,038 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3708 nodes · 8911 edges · 94 communities detected
- Extraction: 48% EXTRACTED · 52% INFERRED · 0% AMBIGUOUS · INFERRED: 4628 edges (avg confidence: 0.58)
...
9. `ChromeLauncher` - 114 edges
10. `SurveyOpener` - 109 edges

## Surprising Connections (you probably didn't know these)
- `Test error handling when API call fails.` --uses--> `SemanticAnalyzer`  [INFERRED]
  tests/test_semantic_engine.py → src/stealth_sync/semantic_engine.py
- `Test that API failures don't crash the system.` --uses--> `SemanticAnalyzer`  [INFERRED]
  tests/test_semantic_engine.py → src/stealth_sync/semantic_engine.py
...
Cohesion: 0.02
Nodes (272): Exception, patch(), ActionSelector, Select actions from a CompactSnapshot using heuristics (no LLM)., Read current balance with exponential backoff.          Dashboard DOM updates as, _get_stealth_js(), inject_stealth_to_tab(), Load stealth injection bundle, or fallback to inline minimal overrides. (+264 more)
...
Cohesion: 0.01
Nodes (155): CookieManager, ╔══════════════════════════════════════════════════════════════════════════════╗, Verwaltet Browser-Cookies für Session-Persistenz.          Extrahiert, speichert, Initialisiert den Cookie-Manager.                  ABLAUF:         1. Speichere, Extrahiert alle Cookies der aktuellen Page.                  ABLAUF:         1., Speichert Cookies in eine JSON-Datei.                  ABLAUF:         1. Erstel, Lädt Cookies aus einer JSON-Datei.                  ABLAUF:         1. Erstelle, Injiziert Cookies in einen Browser-Context.                  ABLAUF:         1. (+147 more)
...
Cohesion: 0.02
Nodes (122): ensure_accessibility(), grant_accessibility(), is_accessibility_enabled(), launch_chrome_with_accessibility(), ================================================================================, DEPRECATED: Use ChromeLauncher.launch_and_verify() instead., Ensure Chrome is running with Accessibility enabled.      Call this ONCE at daem, Start cua-driver daemon if not running. (+114 more)
...
Cohesion: 0.05
Nodes (33): build_graph(), create_graph(), ================================================================================, Entscheide welche Node als nächstes ausgeführt wird.      Dies ist das zentrale, Baue den Survey-StateGraph.      Graph-Struktur:       START → ensure_chrome → r, Factory: Erstelle und kompiliere den Survey-Graph.      Convenience-Wrapper für:, Standalone Survey-Loop ohne LangGraph.      Fallback für Umgebungen wo LangGraph, route() (+25 more)
...
Cohesion: 0.06
Nodes (8): Test survey/autodoc.py — append-only logging and summary generation.  WARUM: Aut, End-to-end: log + regenerate summary from same tmp dir., TestAutodocIntegration, TestLogDecision, TestLogEarnings, TestLogError, TestLogSession, TestPrintSummary
...
- **Thin community `Community 37`** (10 nodes): `test_action_selector.py`, `TestActionSelector`, `._make_snapshot()`, `.test_empty_snapshot()`, `.test_fills_textarea()`, `.test_finds_submit_button()`, `.test_max_two_actions()`, `.test_selects_first_radio_when_no_preferred()`, `.test_selects_preferred_persona_answer()`, `.test_skips_disabled_elements()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 44`** (6 nodes): `get()`, `jitter()`, `patch()`, `rng()`, `safe()`, `stealth_main.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 46`** (5 nodes): `get()`, `jitter()`, `rng()`, `safe()`, `injection.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Why does `patch()` connect `Community 0` to `Community 1`, `Community 2`, `Community 5`, `Community 9`, `Community 10`, `Community 12`, `Community 46`, `Community 14`, `Community 16`, `Community 18`, `Community 22`, `Community 24`, `Community 26`, `Community 27`, `Community 28`, `Community 29`?**
  _High betweenness centrality (0.187) - this node is a cross-community bridge._
- **Why does `SurveyOpener` connect `Community 1` to `Community 0`, `Community 17`, `Community 2`, `Community 5`?**
  _High betweenness centrality (0.154) - this node is a cross-community bridge._
- **Why does `SurveyRunner` connect `Community 0` to `Community 1`, `Community 34`, `Community 4`, `Community 11`, `Community 14`, `Community 26`?**
