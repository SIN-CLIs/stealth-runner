...
## Corpus Check
- 168 files · ~2,712,925 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3534 nodes · 8282 edges · 75 communities detected
- Extraction: 50% EXTRACTED · 50% INFERRED · 0% AMBIGUOUS · INFERRED: 4118 edges (avg confidence: 0.58)
...
3. `BatchExecutor` - 205 edges
4. `SurveyResult` - 166 edges
5. `CompactSnapshot` - 156 edges
6. `patch()` - 124 edges
7. `OutputGenerator` - 108 edges
8. `ChromeLauncher` - 100 edges
9. `CDPConnection` - 100 edges
10. `NIMClient` - 96 edges
...
Cohesion: 0.02
Nodes (246): Exception, ActionSelector, Select actions from a CompactSnapshot using heuristics (no LLM)., print_summary(), Print a formatted summary., Read current balance with exponential backoff.          Dashboard DOM updates as, cmd_balance(), cmd_kill() (+238 more)

### Community 1 - "Community 1"
Cohesion: 0.02
Nodes (207): build_graph(), create_graph(), ================================================================================, Entscheide welche Node als nächstes ausgeführt wird.      Dies ist das zentrale, Baue den Survey-StateGraph.      Graph-Struktur:       START → ensure_chrome → o, Factory: Erstelle und kompiliere den Survey-Graph.      Convenience-Wrapper für:, Standalone Survey-Loop ohne LangGraph.      Fallback für Umgebungen wo LangGraph, route() (+199 more)

### Community 2 - "Community 2"
Cohesion: 0.02
Nodes (165): ABC, BaseSettings, BaseSolver, _find_chrome(), Chrome launcher with hardened flags for stealth automation.  WARUM: Standard-Chr, Manages a Chrome process with stealth flags and isolated profile.      Usage:, Launch Chrome with stealth flags.          Blocks until the CDP endpoint becomes, Open a URL in the default tab (creates one if none exist).          Requires tha (+157 more)

### Community 3 - "Community 3"
Cohesion: 0.02
Nodes (127): jitter(), patch(), rng(), main(), Main daemon module for stealth-sync.  This module implements the core daemon tha, Start the daemon and begin polling.                  This method sets up signal, Stop the daemon gracefully.                  Shuts down the scheduler and perfor, Handle shutdown signals (SIGINT, SIGTERM).                  Args:             si (+119 more)

### Community 4 - "Community 4"
Cohesion: 0.02
Nodes (79): BatchExecutor, capture_dom_hash(), cdp_click_element_by_text(), cdp_keyboard_enter(), detect_language_page(), SOTA: Find element by text + CDP click with verification.      Strategy:     1., Detect Qualtrics language selection page and return select action.      PROBLEM:, Execute batched survey actions via CDP WebSocket.      SOTA features:     - Stat (+71 more)
...
Cohesion: 0.02
Nodes (145): ensure_accessibility(), grant_accessibility(), is_accessibility_enabled(), launch_chrome_with_accessibility(), ================================================================================, DEPRECATED: Use ChromeLauncher.launch_and_verify() instead., Ensure Chrome is running with Accessibility enabled.      Call this ONCE at daem, Start cua-driver daemon if not running. (+137 more)
...
Cohesion: 0.03
Nodes (79): api_close_survey(), api_fill_survey(), api_open_survey(), api_purespectrum_preflight(), api_rate_survey(), CloseSurveyRequest, CloseSurveyResponse, FillSurveyRequest (+71 more)
...
Cohesion: 0.05
Nodes (77): CDPConnection, CDPConnectionError, CDPError, create_cdp(), ================================================================================, Initialize CDP connection.          Args:             ws_url: WebSocket debugger, Connect to the CDP WebSocket with retry., Close the WebSocket connection. (+69 more)
...
Cohesion: 0.05
Nodes (53): ProviderAdapter, CompletionState, ProviderAdapter, ProviderAdapter interface for provider-specific survey behavior.  The engine sho, Provider completion classification., Base adapter with URL matching, commands, and completion detection., Return True if this adapter owns the URL or page text., Return CDP command templates for this provider. (+45 more)
...
Cohesion: 0.13
Nodes (3): detect_completion(), Test detect_completion() — provider-specific completion detection., TestDetectCompletion
...
Cohesion: 0.4
Nodes (5): get_logger(), init_telemetry(), Structured logging with structlog + optional OpenTelemetry tracing.  WARUM: Debu, Initialize structlog once. Idempotent., Get a structlog logger. Init telemetry first if needed.
...
## Knowledge Gaps
- **621 isolated node(s):** `Map old root flags to canonical survey-cli subcommands.`, `Load `survey-cli/survey.py` without confusing it with the `survey` package.`, `Try to delegate to the running Agent-Toolbox API on localhost:8000.`, `Delegate to the canonical survey-cli main() or Agent-Toolbox API.`, `Get Chrome Safe Storage key from macOS Keychain.` (+616 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **Thin community `Community 39`** (10 nodes): `test_action_selector.py`, `TestActionSelector`, `._make_snapshot()`, `.test_empty_snapshot()`, `.test_fills_textarea()`, `.test_finds_submit_button()`, `.test_max_two_actions()`, `.test_selects_first_radio_when_no_preferred()`, `.test_selects_preferred_persona_answer()`, `.test_skips_disabled_elements()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 46`** (6 nodes): `get()`, `jitter()`, `patch()`, `rng()`, `safe()`, `stealth_main.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Why does `patch()` connect `Community 3` to `Community 0`, `Community 4`, `Community 6`, `Community 9`, `Community 12`, `Community 13`, `Community 14`, `Community 17`, `Community 18`, `Community 22`, `Community 24`, `Community 25`, `Community 27`, `Community 28`, `Community 29`?**
  _High betweenness centrality (0.204) - this node is a cross-community bridge._
- **Why does `SurveyRunner` connect `Community 0` to `Community 1`, `Community 4`, `Community 5`, `Community 6`, `Community 36`, `Community 10`, `Community 13`?**
  _High betweenness centrality (0.161) - this node is a cross-community bridge._
- **Why does `SurveyOpener` connect `Community 1` to `Community 0`, `Community 3`, `Community 12`?**
...
- **Are the 195 inferred relationships involving `BatchExecutor` (e.g. with `TestSimpleActions` and `TestRunSurveyLoopDetection`) actually correct?**
  _`BatchExecutor` has 195 INFERRED edges - model-reasoned connections that need verification._
- **Are the 164 inferred relationships involving `SurveyResult` (e.g. with `TestCompleteImmediately` and `TestCircuitBreaker`) actually correct?**
  _`SurveyResult` has 164 INFERRED edges - model-reasoned connections that need verification._