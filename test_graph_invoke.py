...
def get_dashboard_ws():
    ...
    try:
        pages = json.loads(
            urllib.request.urlopen(f"http://127.0.0.1:{CDP_PORT}/json/list", timeout=5).read()
        )
        for p in pages:
...
"""
    try:
        ...
        ws.send(
            json.dumps({"id": 1, "method": "Runtime.evaluate", "params": {"expression": scan_js}})
        )
        r = json.loads(ws.recv())
        ws.close()
...
def main():
    ...
    if not valid:
        ...
        sys.exit(1)
    print("  ✓ Session valid (found 'abmelden/umfragen')")
    ...
    for s in surveys[:5]:
        print(
            f"    - ID:{s['id'][:12]}... {s['provider']:12s} +{s['reward']:.2f}€ {s['title'][:40]}"
        )
    ...
    print(
        f"  ✓ Selected: ID={best['id']}, Provider={best['provider']}, Reward=+{best['reward']:.2f}€"
    )
    ...
    try:
        ...
        print(
            f"Iterations:    {state_dict.get('iteration', 0)}/{state_dict.get('max_iterations', 15)}"
        )
