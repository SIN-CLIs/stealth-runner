...
def run_doctor(repo: Path) -> dict:
    ...
    # ── history.md: Context Writer (NEU) ──────────────────────
    # Liest git log + diff --stat seit letztem Eintrag,
    # erstellt aktuelle history.md Sektion mit vollem Kontext.
    try:
        ...
        if log.stdout.strip() and diff.stdout.strip():
            commits = []
            for line in log.stdout.strip().split("\n")[:10]:
                commits.append(line)
            changes = diff.stdout.strip().split("\n")
            
            history = repo / "history.md"
            today = __import__("datetime").datetime.now().strftime("%Y-%m-%d")
            
            # Extract type from last commit message (feat/fix/docs/chore)
            ...
            # Generate short summary from commit subjects
            subjects = []
            for c in commits:
                parts = c.split(" ", 1)
                if len(parts) > 1:
