...
class TestDragDropAngularSelf(unittest.TestCase):
    ...
    def test_extract_number_with_mock(self):
        ...
        with patch("websocket.create_connection") as mock_ws:
            ...
            mock_conn.recv.return_value = json.dumps({"result": {"result": {"value": "20"}}})
...
def run_live_test(ws_url: str):
    ...
    from drag_drop_angular import DEBUG, solve_drag_puzzle_new
...
def main():
    ...
    parser.add_argument(
        "--self-test", action="store_true", help="Run self-tests without live Chrome"
    )
    ...
    parser.add_argument(
        "--auto-discover", action="store_true", help="Auto-discover purespectrum tab"
    )
    ...
    if args.self_test or (not args.live and not args.ws_url and not args.auto_discover):
        ...
        if result.wasSuccessful():
            ...
            print(
                "  3. Or specify ws-url: python3 test_drag_drop_angular.py --live --ws-url ws://..."
            )
            return 0
        else:
            print()
