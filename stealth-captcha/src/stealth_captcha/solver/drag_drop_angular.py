...
def _extract_number(ws_url: str) -> str | None:
...
def _try_approach_a_playwright_mouse(ws_url: str, number: str, page_info: dict) -> dict | None:
    """
...
""",
        ...
        text=True,
        timeout=45,
        cwd=None,  # Let subprocess use current working directory
    )

    output = result.stdout.strip()
    stderr = result.stderr.strip()

    _log("Playwright mouse stdout: {output[:500]}")
    if stderr:
        _log("Playwright mouse stderr: {stderr[:500]}")
    ...
    try:
        ...
        if outcome.get("success"):
            ...
            _log("APPROACH A FAILED: {outcome.get('error', 'unknown')}")
            return None
    except json.JSONDecodeError:
        _log("APPROACH A FAILED: Invalid JSON output: {output[:200]}")
        return None


def _try_approach_b_cdp_mouse(ws_url: str, number: str, page_info: dict) -> dict | None:
    """
    ...
    if coords.get("error"):
        _log("APPROACH B FAILED: {coords['error']}")
        return None

    sx, sy = coords["sx"], coords["sy"]
    ex, ey = coords["ex"], coords["ey"]

    _log("Coordinates: source=({sx:.1f}, {sy:.1f}) dest=({ex:.1f}, {ey:.1f})")
    ...
    _log("Dispatched {steps} mouseMoved events")
...
def _try_approach_c_synthetic_pointer_with_delays(
    ws_url: str, number: str, page_info: dict
) -> dict | None:
    """
    ...
    _log("Synthetic pointer dispatch: {dispatch_result}")

    time.sleep(2)

    # Verify
    verify = _verify_solution(ws_url)
    _log("Post-synthetic verify: {verify}")
...
def _try_approach_d_html5_drag_and_dom(ws_url: str, number: str, page_info: dict) -> dict | None:
    """
    ...
    r.get("result", {}).get("result", {}).get("value", "")
    _log("HTML5/DOM result: {result[:500]}")

    time.sleep(2)

    # Verify
    verify = _verify_solution(ws_url)
    _log("Post-DOM verify: {verify}")
...
def solve_drag_puzzle_new(ws_url: str) -> DragDropResult:
    ...
    try:
        ...
        _dbg("Target number: {number}")

        if not number:
            return DragDropResult(
                status="failed", error="No number found in puzzle text", debug_log=debug_log
            )

        # === STEP 2: Verify DOM structure ===
        _dbg("=== STEP 2: Verifying DOM structure ===")
        page_info = _get_page_info(ws_url)
        _dbg("Page info: {json.dumps(page_info, indent=2)[:1000]}")
        ...
        if number not in (page_info.get("numbers") or []):
            return DragDropResult(
                status="failed",
                number=number,
                error="Number {number} not in drag items: {page_info.get('numbers')}",
                details=page_info,
                debug_log=debug_log,
            )
        ...
        _dbg("Final verify state: {json.dumps(final_verify, indent=2)[:500]}")
        ...
        _dbg("TRACEBACK: {traceback.format_exc()[:1000]}")
        return DragDropResult(status="failed", error=str(e)[:500], debug_log=debug_log)
