...
def _cleanup_old_entries():
    ...
    for client_id in list(_rate_store.keys()):
        for endpoint in list(_rate_store[client_id].keys()):
            _rate_store[client_id][endpoint] = [
                t for t in _rate_store[client_id][endpoint] if t > cutoff
            ]
...
async def rate_limit_dependency(request: Request):
    ...
    for max_requests, window in limits:
        ...
        if count >= max_requests:
            ...
            logger.warning(
                f"Rate limit exceeded: {client_id} on {path} ({count}/{max_requests} per {window_str})"
            )
            raise HTTPException(
                status_code=429,
