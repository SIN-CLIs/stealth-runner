...
def extract_cookies(profile_name="Default", user="jeremy", domain_filter=None):
    ...
    if not os.path.exists(chrome_path) and user != os.path.basename(os.path.expanduser("~")):
        chrome_path = (
            f"/Users/{user}/Library/Application Support/Google Chrome/{profile_name}/Cookies"
        )
    ...
    for row in cursor.fetchall():
        ...
        expires_unix = expires / 1000000 - 11644473600 if expires and expires > 0 else -1
...
def main():
    ...
    if args.output is None:
        profile_dir = os.path.expanduser(
            f"~/Library/Application Support/Google/Chrome/{args.profile}"
        )
