...
def _get_survey_runner():
    ...
    try:
        from survey.runner import RunnerConfig, SurveyRunner
    except ImportError as e:
        # Import fehlgeschlagen (wahrscheinlich openai fehlt).
        raise RuntimeError("SurveyRunner requires openai. Install openai package.") from e
...
def _get_auth_flow():
    ...
    try:
        from survey.auth import CuaAdapter, GoogleOAuthFlow, LoginVerifier
    except ImportError as e:
    ...
    try:
        ...
        if req.provider_filter:
            ...
            config.skip_providers = [
                p for p in config.skip_providers if p not in req.provider_filter
            ]
...
async def extract_cookies(req: CookieExtractRequest):
    ...
    try:
        ...
        if req.domain_filter:
            cookies = [c for c in cookies if req.domain_filter in (c.get("domain") or "")]
    ...
    while _background_running:
        try:
            ...
            if not acquire_survey_lock(survey_id):
                print(
                    f"[BG-LOOP] Survey lock active — skipping survey {survey_id} (another survey running)"
                )
            ...
            if earned > 0:
                print(
                    f"[BG-LOOP] SUCCESS: +{earned:.2f}€ (survey={survey_id}, provider={provider})"
                )
            elif final.screen_out:
                print(f"[BG-LOOP] SCREEN-OUT: 0.00€ (survey={survey_id}, provider={provider})")
            elif errors > 0:
                print(
                    f"[BG-LOOP] ERRORS: {errors} errors (survey={survey_id}, provider={provider})"
                )
            else:
