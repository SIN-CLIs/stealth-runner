...
class BrowserStartRequest(BaseModel):
    ...
    profile_name: str = Field(
        default="default", description="Browser profile to use (default: 'default')"
    )
    ...
    headless: bool | None = Field(
        default=None, description="Override headless mode. None=read from BROWSER_HEADLESS env var"
    )
    ...
    cdp_port: int | None = Field(
        default=None, description="CDP debugging port. None = use BrowserManager default (9999)"
    )
...
class BrowserStartResponse(BaseModel):
    ...
    status: Literal["success", "error"] = Field(
        default="success", description="Operation result: 'success' or 'error'"
    )
    ...
    profile: str = Field(description="Active browser profile name")
    ...
    headless: bool = Field(description="Actual headless mode used")
    ...
    cdp_port: int | None = Field(description="CDP port Chrome is listening on")
    ...
    message: str = Field(description="Human-readable status, e.g. 'Browser started (warm)'")
...
class BrowserStopResponse(BaseModel):
    ...
    status: Literal["success", "error"] = Field(
        default="success", description="'success' (even if browser was not running — idempotent)"
    )

    # message: Kurze Bestätigung.
    message: str = Field(description="e.g. 'Browser stopped', 'Browser was not running'")
...
class BrowserHealthResponse(BaseModel):
    ...
    running: bool = Field(description="True if Chrome process is alive and CDP reachable")

    # profile: Aktives Profil (oder None wenn nicht läuft).
    # WARUM Optional? Wenn Browser nicht läuft → kein Profil aktiv.
    profile: str | None = Field(description="Active profile name, or None if not running")
...
class LoginRequest(BaseModel):
    ...
    profile_name: str = Field(
        default="default", description="Browser profile for this login session"
    )
    ...
    headless: bool | None = Field(
        default=None, description="Headless override. None=use env default"
    )
    ...
    timeout_ms: int = Field(
        default=30000, description="Max wait time in milliseconds for login completion"
    )
    ...
    pid: int | None = Field(
        default=None, description="Existing Chrome PID (None = auto-detect/start new Chrome)"
    )
...
class LoginResponse(BaseModel):
    ...
    status: Literal["success", "error", "already_logged_in"] = Field(
        default="success", description="'success', 'already_logged_in', or 'error'"
    )
    ...
    service: str = Field(description="Service name, e.g. 'heypiggy'")

    # profile: Verwendetes Profil.
    profile: str = Field(description="Profile used for login")
    ...
    details: dict[str, Any] | None = Field(
        default=None, description="Technical details: pid, wid, error_info, etc."
    )
...
class SurveyClickCardRequest(BaseModel):
    ...
    survey_id: str | None = Field(
        default=None, description="Specific survey ID, or None = first available survey card"
    )

    # cdp_port: CDP Port.
    # WARUM 9999? Siehe BrowserStartRequest Erklärung.
    cdp_port: int = Field(default=9999, description="CDP port for Chrome communication")
...
class SurveyClickCardResponse(BaseModel):
    ...
    survey_id: str | None = Field(
        default=None, description="Clicked survey ID, or 'first_available', or None"
    )
    ...
    modal_visible: bool = Field(
        default=False, description="True if a modal dialog is visible after click"
    )

    # modal_text: Text-Inhalt des Modals (erste 500 Zeichen).
    # WARUM? Debugging: Was steht im Modal? Titel? Beschreibung?
    modal_text: str | None = Field(
        default=None, description="Modal content text (first 500 chars)"
    )
...
class SurveyElement(BaseModel):
    ...
    text: str = Field(description="Visible text or label of the element")
    ...
    label: str | None = Field(default=None, description="HTML label text, if available")
    ...
    value: str | None = Field(
        default=None, description="HTML value attribute or current content"
    )
    ...
    selected: bool | None = Field(
        default=None, description="True if selected (radio/checkbox), None for other types"
    )
    ...
    visible: bool = Field(default=True, description="True if element is visible on page")
...
class SurveyGetModalResponse(BaseModel):
    ...
    status: Literal["success", "error", "no_modal"] = Field(
        default="success", description="'success', 'no_modal' (survey not started), or 'error'"
    )
    ...
    modal_visible: bool = Field(
        default=False, description="True if a modal dialog is currently visible"
    )

    # elements: Liste aller interaktiven Elemente.
    # WARUM List[SurveyElement]? Typed Array → Client weiß genau was drin ist.
    elements: list[SurveyElement] = Field(
        default_factory=list, description="All interactive elements on current page/modal"
    )

    # text: Gesamter Text-Inhalt der Seite (erste 2000 Zeichen).
    # WARUM? Für Text-Suche, Keyword-Erkennung, Progress-Extraktion.
    text: str = Field(default="", description="Page text content (first 2000 chars)")
    ...
    progress: str | None = Field(
        default=None, description="Survey progress, e.g. '3/10'. None if not detected"
    )

    # message: Human-readable Status.
    message: str = Field(description="e.g. 'Modal with 15 elements', 'No survey modal open'")
...
class SurveyClickButtonRequest(BaseModel):
    ...
    timeout_ms: int = Field(
        default=5000, description="Wait time in milliseconds after click for page to load"
    )
...
class SurveyClickButtonResponse(BaseModel):
    ...
    button_label: str = Field(description="Button label that was clicked or attempted")

    # page_changed: Hat sich die Seite geändert?
    # WARUM? Siehe oben. Kritischer Indikator für Loop-Logik.
    page_changed: bool = Field(
        default=False, description="True if page content changed after click (new question loaded)"
    )
    ...
    new_text: str = Field(default="", description="New page content (first 500 chars) after click")
...
class SurveySelectOptionRequest(BaseModel):
    ...
    wait_after_ms: int = Field(
        default=1000, description="Wait time in milliseconds after selection for UI to update"
    )
...
class SurveySelectOptionResponse(BaseModel):
    ...
    status: Literal["success", "error", "not_found"] = Field(
        default="success", description="'success', 'not_found' (option not found), or 'error'"
    )

    # option_text: Welche Option wurde versucht auszuwählen.
    option_text: str = Field(description="Option text that was selected or attempted")
...
class SurveyFillTextRequest(BaseModel):
    ...
    value: str = Field(..., description="Text value to enter into the field")
...
class SurveyFillTextResponse(BaseModel):
    ...
    status: Literal["success", "error", "not_found"] = Field(
        default="success", description="'success', 'not_found' (field not found), or 'error'"
    )

    # input_label: Welches Feld wurde versucht zu füllen.
    input_label: str = Field(description="Field label that was filled or attempted")

    # value: Eingetragener Wert.
    value: str = Field(description="Value that was entered (echo for confirmation)")

    # message: Human-readable Status.
    message: str = Field(description="e.g. 'Filled: age=32', 'Field not found: phone_number'")
...
class SurveyClickCustomRadioRequest(BaseModel):
    ...
    wait_after_ms: int = Field(
        default=1000, description="Wait time in milliseconds after click for UI to update"
    )
...
class SurveyClickCustomRadioResponse(BaseModel):
    ...
    # status: Ergebnis.
    status: Literal["success", "error", "not_found"] = Field(
        default="success", description="'success', 'not_found' (no matching DIVs), or 'error'"
    )

    # div_class: Verwendete CSS-Klasse (Echo für Bestätigung).
    div_class: str = Field(description="CSS class that was searched for")

    # divs_found: Anzahl gefundener DIVs.
    divs_found: int = Field(default=0, description="Number of matching DIVs found on page")

    # clicked_index: Tatsächlich geklickter Index.
    clicked_index: int = Field(default=-1, description="Index of the DIV that was actually clicked")

    # selected_text: Text des geklickten DIVs (für Debugging).
    selected_text: str | None = Field(
        default=None, description="Text content of the clicked DIV"
    )
...
class SurveyRunOneRequest(BaseModel):
    ...
    survey_id: str | None = Field(
        default=None, description="Survey ID or None = first available survey"
    )
...
class SurveyRunOneResponse(BaseModel):
    ...
    survey_id: str = Field(description="Survey ID that was executed")

    # pages_completed: Anzahl beantworteter Seiten.
    pages_completed: int = Field(default=0, description="Number of survey pages answered")

    # earned: Verdiente Belohnung in EUR.
    earned: float = Field(
        default=0.0, description="Earned reward in EUR (0.0 if screen_out or error)"
    )

    # elapsed_s: Gesamtdauer in Sekunden.
    elapsed_s: float = Field(default=0.0, description="Total execution time in seconds")

    # error: Fehlermeldung.
    # WARUM Optional? Nur bei Status="error" gesetzt.
    # Bei Erfolg → None (sauberes Model).
    error: str | None = Field(
        default=None, description="Error message if status='error', None otherwise"
    )
...
class SurveyRunRequest(BaseModel):
    ...
    provider_filter: str | None = Field(default=None, description="Filter by provider name")
...
class SurveyResult(BaseModel):
    ...
    provider: str | None = Field(default=None, description="Survey provider name")
...
class SurveyRunResponse(BaseModel):
    ...
    results: list[SurveyResult] = Field(
        default_factory=list, description="Individual survey results"
    )
...
class DashboardSurvey(BaseModel):
    ...
    duration_min: int | None = Field(
        default=None, description="Estimated duration in minutes. None if not displayed"
    )
    ...
    title: str = Field(default="", description="Survey title or description (first 100 chars)")
...
class DashboardScanResponse(BaseModel):
    ...
    status: Literal["success", "error"] = Field(
        default="success", description="'success' or 'error'"
    )

    # balance_eur: Aktueller Kontostand.
    balance_eur: float = Field(default=0.0, description="Current account balance in EUR")

    # available_surveys: Liste aller verfügbaren Surveys.
    available_surveys: list[DashboardSurvey] = Field(
        default_factory=list, description="All available surveys with rewards"
    )

    # total_rewards: Summe aller Rewards.
    total_rewards: float = Field(default=0.0, description="Sum of all available rewards in EUR")

    # message: Human-readable Zusammenfassung.
    message: str = Field(description="e.g. 'Found 5 surveys, total rewards: 3.50€'")
...
class BalanceResponse(BaseModel):
    ...
    # balance_eur: Der Betrag.
    balance_eur: float = Field(description="Current balance in EUR")

    # currency: Währung.
    currency: str = Field(
        default="EUR", description="Currency code (ISO 4217). Always 'EUR' for HeyPiggy"
    )

    # message: Human-readable Status.
    message: str = Field(description="e.g. 'Balance: 12.35€', 'Balance not visible'")
...
class CookieExtractRequest(BaseModel):
    ...
    save_to_file: bool = Field(default=True, description="Save extracted cookies to a JSON file")
...
class CookieExtractResponse(BaseModel):
    ...
    cookies: list[dict[str, Any]] = Field(description="List of extracted cookie dictionaries")

    # count: Anzahl.
    count: int = Field(description="Number of cookies extracted")
    ...
    saved_to: str | None = Field(
        default=None, description="Path to saved file, or None if not saved"
    )

    # execution_time: Dauer.
    execution_time: str = Field(default="", description="Execution time, e.g. '0.45s'")
...
class CookieInjectRequest(BaseModel):
    ...
    verify_session: bool = Field(
        default=True, description="Verify session after injection by navigating to dashboard"
    )
...
class CookieInjectResponse(BaseModel):
    ...
    injected_count: int = Field(default=0, description="Number of cookies successfully injected")

    # session_active: Funktioniert die Session?
    session_active: bool = Field(
        default=False, description="True if session is active after injection (logged in)"
    )

    # execution_time: Dauer.
    execution_time: str = Field(default="", description="Execution time, e.g. '1.23s'")
...
class BackupCreateRequest(BaseModel):
    ...
    working_filename: str = Field(
        default="heypiggy-cookies.json", description="Cookie filename in working dir"
    )
...
class ScreenshotRequest(BaseModel):
    ...
    full_page: bool = Field(
        default=False, description="True = full page screenshot, False = viewport only"
    )

    # selector: CSS-Selector für Element-Screenshot.
    # WARUM Optional? None = ganze Seite.
    selector: str | None = Field(
        default=None, description="CSS selector to screenshot a specific element. None = full page"
    )
...
class ScreenshotResponse(BaseModel):
    ...
    base64_image: str = Field(description="Screenshot as base64-encoded PNG string")

    # mime_type: Format.
    mime_type: str = Field(
        default="image/png", description="Image MIME type. Always 'image/png' currently"
    )
...
class PageContentRequest(BaseModel):
    ...
    selector: str | None = Field(
        default=None, description="CSS selector to extract specific element. None = entire page"
    )
    ...
    max_length: int = Field(
        default=5000, ge=1, le=50000, description="Max text length to return (1-50000 chars)"
    )
...
class PageContentResponse(BaseModel):
    ...
    # url: Aktuelle URL.
    url: str = Field(description="Current page URL after extraction")

    # title: Seiten-Titel.
    title: str = Field(description="Current page title")

    # text: Extrahierter Text.
    text: str = Field(description="Extracted text content (first max_length chars)")

    # html_length: Länge des HTML.
    html_length: int = Field(description="Length of full HTML source (for complexity indicator)")
...
class ErrorResponse(BaseModel):
    ...
    status: Literal["error"] = Field(default="error", description="Always 'error'")
...
class WorkflowRunBestRequest(BaseModel):
    ...
    cdp_port: int = Field(
        default=9999, description="CDP port for Chrome communication. Stealth-Runner standard: 9999"
    )
    max_reward_filter: float = Field(
        default=0.0, description="Minimum reward in EUR to consider (0.0 = no filter)"
    )
    strategy: Literal["reward", "efficiency", "duration"] = Field(
        default="efficiency", description="Survey selection strategy"
    )
...
class WorkflowRunBestResponse(BaseModel):
    ...
    status: Literal["success", "no_surveys", "session_expired", "error"] = Field(
    ...
    survey_selected: DashboardSurvey | None = Field(
        default=None, description="Selected survey (if any)"
    )
    card_clicked: bool = Field(default=False, description="Was survey card clicked successfully")
    modal_buttons: list[str] = Field(
        default_factory=list, description="Buttons visible in modal after click"
    )
