...
class TextCaptchaRequest(BaseModel):
    ...
    image_selector: str = Field(
        default="img.captcha-image, .captcha-image img", description="Captcha-Bild Selector"
    )
    input_selector: str = Field(
        default="input[name='captcha'], input.captcha-input", description="Text-Input Selector"
    )
    submit_selector: str = Field(
        default="button[type='submit'], .captcha-submit", description="Submit-Button Selector"
    )
...
class TextCaptchaResponse(BaseModel):
    ...
    status: Literal["ok", "error"] = Field(description="Ergebnis-Status")
    solved: bool = Field(description="Ob das Captcha gelöst wurde")
    solution: str | None = Field(default=None, description="Erkannter Text")
...
class AngularDragDropRequest(BaseModel):
    ...
    target_number: str | None = Field(
        default=None, description="Ziel-Zahl (auto-detect wenn None)"
    )
...
class AngularDragDropResponse(BaseModel):
    ...
    number: str | None = Field(default=None, description="Die gezogene Zahl")
    error: str | None = Field(default=None, description="Fehler-Details")
    elapsed_ms: float = Field(default=0.0, description="Laufzeit in ms")
...
def _get_text_solver():
    ...
    try:
        from stealth_captcha.solver.base import SolveOutcome
        from stealth_captcha.solver.text import PixtralLargeOCR, TextCaptchaSolver
...
async def solve_slide_captcha(req: SlideCaptchaRequest) -> SlideCaptchaResponse:
    ...
    try:
        ...
        return SlideCaptchaResponse(
            ...
            or ("solved" if result.outcome == SolveOutcome.SUCCESS else "failed"),
        )
