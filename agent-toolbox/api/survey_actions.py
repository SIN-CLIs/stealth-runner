...
def get_dashboard_ws(port: int) -> Optional[str]:
    """
...
def ws_eval(ws_url: str, js: str, timeout: int = 10) -> Optional[dict]:
    """
...
def ws_eval_multi(ws_url: str, *calls) -> List[dict]:
    ...
    try:
        ...
        for i, (js_code, call_id) in enumerate(calls):
            ws.send(json.dumps({
...
"""
    ...
    return SurveyClickCardResponse(
        ...
        message=f"Clicked survey card" + (f" ({survey_id})" if survey_id else ""),
    )
...
"""
    ...
    # Parse Antwort.
    parsed = json.loads(response)
    modal_data = json.loads(
        parsed.get("result", {}).get("result", {}).get("value", "{}")
    )
    ...
    for el in elements.get("elements", []):
        element_list.append(SurveyElement(
            ref=el.get("ref", ""),
            role=el.get("role", ""),
            text=el.get("text", ""),
...
async def run_one_survey(req: SurveyRunOneRequest):
    ...
    click_result = await click_card(SurveyClickCardRequest(
...
""",
    ...
    for page_num in range(max_pages):
        ...
        completion_keywords = ["danke", "fertig", "vielen dank", "complete", "done", "abgeschlossen"]
        ...
        disqualification_keywords = ["screen out", "leider", "nicht qualifiziert", "disqualifiziert", "abgebrochen"]
        ...
        for el in elements.get("elements", []):
            if el.get("role") == "textbox" and not el.get("value"):
                ...
                ws.send(json.dumps({
                    ...
                    "method": "Runtime.evaluate",
                    "params": {
                        "expression": f"document.querySelector('input[type=\"text\"]').value = 'test';",
                        "returnByValue": True
                    }
                }))
                ws.recv()
